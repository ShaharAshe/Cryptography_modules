...
class VigenereCipher:
    def __init__(self, text:str, AA:float, BB:float, CC:float, char_1:str="A", char_2:str="B", char_3:str="C", time:int=2) -> None:
        self.__AA:float = AA
        self.__BB:float = BB
        self.__CC:float = CC

        self.__char_1:str = char_1
        self.__char_2:str = char_2
        self.__char_3:str = char_3

        self.__text:str = text
        self.__decrypt_text:str = text
        self.__text_str:list = [c for c in text if c.isalpha()]
        self.__first_frequencies_vector:list = self.__text_str[:]
    ...
    def __get_frequencies_vector(self):
        for i in range(len(self.__text_str)):
            if self.__text_str[i] == 0:
                self.__first_frequencies_vector.append(self.__AA)
            elif self.__text_str[i] == 1:
                self.__first_frequencies_vector.append(self.__BB)
            elif self.__text_str[i] == 2:
                self.__first_frequencies_vector.append(self.__CC)
    ...
    def get_decrypt_text(self):
        ...
        for c in self.__right_text_str:
            ...
            temp_key_lst:list = []
            if len(c):
                temp_dec_lst.append(str(chr(((ord(c[0].lower())-ord('a')+self.__decrypt_key[0])%3)+ord('a')).upper()))
                temp_key_lst.append(str(c[0]))
            if len(c) > 1:
                temp_dec_lst.append(str(chr(((ord(c[1].lower())-ord('a')+self.__decrypt_key[1])%3)+ord('a')).upper()))
                temp_key_lst.append(str(c[1]))
            if len(c) > 2:
                temp_dec_lst.append(str(chr(((ord(c[2].lower())-ord('a')+self.__decrypt_key[2])%3)+ord('a')).upper()))
    ...
    def __decrypt(self):
        A1 = [self.__AA, self.__BB, self.__BB]
        A2 = A1[2:]+A1[:2]
        A3 = A2[2:]+A2[:2]

        print(f"{self.__right_text_str = }")
        ...
        for i in range(len(first_leteer)):
            if first_leteer[i].lower() == self.__char_1.lower():
                V1[0] += 1
            elif first_leteer[i].lower() == self.__char_2.lower():
                V1[1] += 1
            elif first_leteer[i].lower() == self.__char_3.lower():
                V1[2] += 1
        sumv = sum(V1)
        ...
        for i in range(len(second_leteer)):
            if second_leteer[i].lower() == self.__char_1.lower():
                V2[0] += 1
            elif second_leteer[i].lower() == self.__char_2.lower():
                V2[1] += 1
            elif second_leteer[i].lower() == self.__char_3.lower():
                V2[2] += 1
        sumv = sum(V2)
        ...
        for i in range(len(third_leteer)):
            if third_leteer[i].lower() == self.__char_1.lower():
                V3[0] += 1
            elif third_leteer[i].lower() == self.__char_2.lower():
                V3[1] += 1
            elif third_leteer[i].lower() == self.__char_3.lower():
                V3[2] += 1
        sumv = sum(V3)
...
if __name__ == '__main__':
    ...
    cipher = VigenereCipher(Encrypted_text, AA, BB, CC, char_1, char_2, char_3, min_sift)
    cipher.process()
